...
class LocalGroceryStore:
    ...
    def add_to_cart(self, item):
        """
        Creating shopping cart for user where user can add items from the list according to their names and select
        the quantity .
        User can only add upto 100 of each item .
        User can only add valid qantity in numbers or program will ask for valid amount and numbers.
    ...
    def start_shopping(self):
        ...
        User will be provided the list of items from shop where user can enter the items name to add to their cart.
        At the start of program user will be provide with options of wheter the user wants to shop or not, if the user wants to shop they
        can continue adding to their cart.
        If the user doesn't wants to shop and enters 'exit' the program will end and user receives their sum-total and goodbye message.
        """
        print("Welcome to the Grocery Store!")
        self.display_items()
        while True:
            choice = input("Enter the item you want to add to cart (or 'exit' to end shopping): ").lower()
            if choice == 'exit':
                break
            elif choice.isdigit():
        ...
        if self.cart:
            while True:
                remove_choice = input("Do you want to remove items from your cart? (y/n): ").lower()
                if remove_choice == 'y':
                    item_to_remove = input("Enter the item you want to remove from your cart: ").lower()
